...
const TableRow = ({ resource, currentPath, postType, resourceID, displayConfig }) => {
...
return (
        <tr key={resource.id}>
        {displayConfig.title && <td style={{textAlign: 'left', }}><Link style={{textDecoration: 'none'}} to={allPaths.get(resource.id)}>{resource.title}</Link></td>}
